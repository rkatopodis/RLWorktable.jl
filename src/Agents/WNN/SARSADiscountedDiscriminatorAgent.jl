...
function SARSADiscountedDiscriminatorAgent(::Type{O}, actions::StaticArray{Tuple{AS},A,1}, steps, n, forgetting_factor, discount, ϵ, encoder::E; seed::Union{Nothing,Int}=nothing) where {AS,OS,T,O <: StaticArray{Tuple{OS},T,1},A,E <: AbstractEncoder{T}}
    ...
    SARSADiscountedDiscriminatorAgent{AS,OS,T,O,A,E}(steps, actions, discount, ϵ, Q̂, buffer, nothing, false, nothing, rng)
end

function SARSADiscountedDiscriminatorAgent(
    env, encoder::E; steps, n, forget::Float64=1.0, gamma::Float64=1.0, epsilon::Float64=0.01, seed::Union{Nothing,Int}=nothing) where {T <: Real,E <: AbstractEncoder{T}}
    ...
    SARSADiscountedDiscriminatorAgent(observation_type(env), action_set(env), steps, n, forget, gamma, epsilon, encoder; seed)
end
...
function reset!(agent::SARSADiscountedDiscriminatorAgent; seed::Union{Nothing,Int}=nothing)
    if !isnothing(seed)
        if seed ≥ 0
        seed!(agent.rng, seed)
        else
