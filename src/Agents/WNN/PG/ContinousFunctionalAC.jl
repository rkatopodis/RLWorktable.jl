...
function ContinousFunctionalAC(env, encoder::E; steps, n, η, epochs, γ, forgetting_factor=1.0, cov::C, seed=nothing) where {T <: Real,E <: AbstractEncoder{T},C <: AbstractMatrix{T}}
    # ContinousFunctionalAC{action_length(env),T,observation_type(env),action_type(env),E,C}(n, observation_length(env), η, γ, encoder, cov; seed)
    ContinousFunctionalAC(observation_type(env), action_type(env), steps, n, η, epochs, γ, forgetting_factor, encoder, cov; seed)
end
